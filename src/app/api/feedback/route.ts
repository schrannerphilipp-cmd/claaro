...
export async function POST(req: NextRequest) {
  let body: { name: string; email: string; kategorie: string; nachricht: string; hauptaccountId?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const { name, email, kategorie, nachricht } = body;
  ...
  const feedbackEmail = process.env.FEEDBACK_EMAIL ?? "feedback@claaro.de";
  ...
  if (resend) {
    try {
      await resend.emails.send({
        ...
        to: feedbackEmail,
        subject: `Claaro Feedback: ${kategorie} von ${name}`,
        html: `
